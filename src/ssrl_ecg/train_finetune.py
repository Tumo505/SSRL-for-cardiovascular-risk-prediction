...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Fine-tune classifier from SSL-pretrained encoder.")
...
def main() -> None:
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        running = 0.0
        n = 0
        pbar = tqdm(train_loader, desc=f"FineTune Epoch {epoch}/{args.epochs}")
        for x, y in pbar:
            x = x.to(device)
            y = y.to(device)
